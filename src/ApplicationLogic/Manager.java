...
public class Manager {
	...
	public void resetStation(){
		...
		HTMLSites.taString.setLength(0);
	...
	public void endGame(){
		...
		for(Player p: players){
			for(int i=0; i<players.size(); i++){
				if(podium[i]==p)
					break;
				if(i==players.size()-1 && podium[i]==null){
					podium[i]= p;
					end= true;
				}
			}
	...
	public static String stringFColorG(int color){
		return color==1?"Grün":(color==2?"Gelb":(color==3?"Blau":"Rot"));
	}
...
//			}
		...
		if(checkEndGame(currentPlayer)){
			...
			int place= 0;
			while(podium[place]!=currentPlayer)
				place++;
			place+=1;
			
			pasteInfo(german==true?("\n"+currentPlayer.getPlayerName()+"("+stringFColorG(currentPlayer.getPlayerColor())+") erlangt den\n"
					+(place==1?"ersten":(place==2?"zweiten":(place==3?"dritten":"vierten")))+" Platz\n\n")
					:("\n"+currentPlayer.getPlayerName()+"("+stringFColor(currentPlayer.getPlayerColor())+") got on the\n"
					+(place==1?"first":(place==2?"second":(place==3?"third":"fourth")))+" place\n\n")
					);
	...
	public void nextPlayer() { // COULD BE INTEGRATED INTO MANAGEGAME
		if(gameActive!=false){
			if (firstRound || (currentPlayer.getChips().size()==0 || diceValue != 6) ){
				do{
...
//			player.getChips().get(0).setMergeCount(1);
	...
	public int rollDice(String IP) throws NoPermissionException {
		...
		if (firstRound) {
			...
			pasteInfo(german==true?("\n"+currentPlayer.getPlayerName()+"("+stringFColorG(currentPlayer.getPlayerColor())+") würfelte eine "+diceValue+"\n\n")
					:("\n"+currentPlayer.getPlayerName()+"("+stringFColor(currentPlayer.getPlayerColor())+") just rolled a "+diceValue+"\n\n")
					);
			...
			if (players.indexOf(currentPlayer) == players.size()-1) {
				...
				pasteInfo(german==true?("\n"+currentPlayer.getPlayerName()+"("+stringFColorG(currentPlayer.getPlayerColor())+")\nhat den höchsten Wurf und fängt an\n\n")
						:("\n"+currentPlayer.getPlayerName()+"("+stringFColor(currentPlayer.getPlayerColor())+")\ngot the highest dicevalue and starts\n\n")
						);
				diceRolled = false;
				firstRound = false;
			...
			currentPlayer.setLastDiceValue(diceValue);
			pasteInfo(german==true?("\n"+currentPlayer.getPlayerName()+"("+stringFColorG(currentPlayer.getPlayerColor())+") würfelte eine "+diceValue+"\n\n")
					:("\n"+currentPlayer.getPlayerName()+"("+stringFColor(currentPlayer.getPlayerColor())+") rolled a "+diceValue+"\n\n")
					);
			diceRolled = true;
			...
			if(!playerCanMakeAction() && --throwCount==0){
				...
				if(!gotActiveChips(currentPlayer, 0) && gotInactiveChips(currentPlayer) )
					diceRolled= false;
				else{
					nextPlayer();
					diceRolled = false;
	...
	public boolean playerCanMakeAction(){
		int notMovable= 0;
		...
		if(diceValue!= 6 && notMovable==currentPlayer.getChips().size())
			return false;
		else if(diceValue==6 && notMovable!=0 /*|| (diceValue==6 && notMovable==(currentPlayer.getChips().size()-onStairs.size()))*/)
			return true;
		
		for(Chip chip: onStairs){
			if(chip.getPosition()+diceValue <= getMyStairs(currentPlayer).length)
				return true;
		}
	...
	public Thread KI(/*final int rolledV,*/ final boolean started) {
		...
		Runnable z = new Runnable() {
			private boolean keepSwitching= true;
						...
						if(currentPlayer.getChips().size()!=0){
							...
							if (firstRound) {
								...
								do{
									...
									currentPlayer.setLastDiceValue(diceValue);
									textOutPut+= german==true?("\n"+currentPlayer.getPlayerName()+"("+stringFColor(currentPlayer.getPlayerColor())+ ") würfelte eine "+diceValue)
											:("\n"+currentPlayer.getPlayerName()+"("+stringFColor(currentPlayer.getPlayerColor())+ ") rolled a "+diceValue);
									if(playerCanMakeAction()){
										...
										if(checkEndGame(currentPlayer)){
											...
											while(podium[place]!=currentPlayer)
												place++;
											place+=1;
											textOutPut= german==true?("\n"+currentPlayer.getPlayerName()+"("+stringFColor(currentPlayer.getPlayerColor())+") erlangt den\n"
													+(place==1?"ersten":(place==2?"zweiten":(place==3?"dritten":"vierten")))+" Platz\n\n")
													:("\n"+currentPlayer.getPlayerName()+"("+stringFColor(currentPlayer.getPlayerColor())+") got on the\n"
													+(place==1?"first":(place==2?"second":(place==3?"third":"fourth")))+" place\n\n")+textOutPut;
...
//										}		
						...
						if((!firstRound) || (firstRound /*&& players.indexOf(currentPlayer)!=players.size()-1*/ && keepSwitching))
							currentPlayer = players.get((players.indexOf(currentPlayer)+1) % players.size());
						else if(firstRound && !keepSwitching /*&& players.indexOf(currentPlayer)==players.size()-1*/){
							firstRound = false;
							keepSwitching= true;
						}
				}
			...
			private synchronized void innerKI() {
				...
				try {
					if (kick != -1) {
						textOutPut+=german==true?(" und \nwarf einen Chip raus\n\n"):(" and \nkicked a chip\n\n");
						moveChip(/*rolledV*/diceValue, kick);
					} else if (merge != -1) {
						textOutPut+=german==true?(" und \nverschmolz seine Chips\n\n"):(" and \nmerged his chips\n\n");
			...
			private int checkGoHome(int value) {
				for (Chip chip : currentPlayer.getChips()) {
					if (chip.getStatus() == 1
						...
						if(chip.getPosition() == currentPlayer.getEndPosition() && value==6)
							textOutPut+= german==true?("\nund brachte einen Chip ins Ziel\n\n"):("\nand got a chip into goal\n\n");
						else
							textOutPut+= german==true?("\nund brachte einen Chip auf seine Treppe\n\n"):("\nand got a chip on his stairs\n\n");
						return currentPlayer.getChips().indexOf(chip);
					}
					if (chip.getStatus() == 2
							&& chip.getPosition() + value <= 5) {
						
						if(chip.getPosition() + value == 5)
							textOutPut+= german==true?("\nund brachte einen Chip ins Ziel\n\n"):("\nand got a chip into goal\n\n");
						else
							textOutPut+= german==true?("\nund bewegte einen Chip auf seinen Treppen\n\n"):("\nand moved a chip on the stairs\n\n");
						return currentPlayer.getChips().indexOf(chip);
					}
				}
				return -1;
						...
						if (chip.getStatus() == 0
								&& (gameArea.getField()[currentPlayer
										...
										: gameArea.getField()[currentPlayer
												...
												.getPlayerColor())) {
							textOutPut+= german==true?("\nund brachte einen Chip raus\n\n"):("\nand moved a chip out of home\n\n");
							return currentPlayer.getChips().indexOf(chip);
						}
					}
				}
	...
	public boolean gotActiveChips(Player player, int mode){
		switch(mode){
		case 0:
			for(Chip chip: currentPlayer.getChips()){
...
//			action.setVisible(true);
	...
	public String getCenterField(String color){
		int chipMerges = 0;
		int chipsTotal= 0;
		...
		while(itP.hasNext()){
			...
			if(Manager.stringFColor(actualP.getPlayerColor()).equals(color)){
				notNull= true;
				chipsTotal= actualP.getChips().size();
				for(Chip c:actualP.getChips()){
					chipMerges+= c.getMergeCount(); 
				}
			}			
		}
		return (notNull!=false?Integer.toString(4-chipMerges-chipsTotal):"");
	}
